...
for (f in 1:length(fileNames)) {
  ...
  load(fileNames[f])
  tab[f, 5:13] <- round(colMeans(rslt_all$gp[, 1:9] < 0.05, na.rm = T), 4)
  tab[f, 14] <- round(mean(rowMins(cbind(rslt_all$gp[, 6:9] * 4, 1), na.rm = T)  < 0.05), 4) 
  tab[f, 15:22] <- round(colMeans(rslt_all$gp[, 10:17] < 0.05, na.rm = T), 4)
}
