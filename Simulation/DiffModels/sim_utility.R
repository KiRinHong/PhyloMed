...
# Test alpha in quasi-likelihood model, model count
.test_alpha_qsc <- function(Mc, Trt, conf, est.reduce.beta, test.type="mv"){
...
# same as the qr one
.test_alpha_lr <- function(G, Trt, covariates, resid, disp){
  Trt = matrix(Trt, nrow = length(Trt), ncol = 1)
  X1 = model.matrix(G~0+covariates)
  # continuous traits
  W = t(Trt) %*% Trt-
    (t(Trt) %*% X1) %*%
    solve(t(X1) %*% X1) %*% (t(X1) %*% Trt)
...
.test_alpha_qsg <- function(G, Trt, covariates, resid, disp){
  Trt = matrix(Trt, nrow = length(Trt), ncol = 1)
  X1 = model.matrix(G~0+covariates)
