rm(list = ls())
setwd("~/Project/QH0020/medtest/PhyloMed/Revised/DiffModels/Result/")
fileNames <- list.files("./", pattern = ".*.Rdata")
tab <- matrix(NA, nrow = length(fileNames), ncol = 7, dimnames = list(c(),
                                                                       c("CausalType", "N", "A", "B", "LR", "DM", "QS")))
for (f in 1:length(fileNames)) {
  print(fileNames[f])
  tab[f, 1:4] <- as.numeric(sub("[.]$", "", unlist(strsplit(fileNames[f], split = "[aA-zZ]+"))[-1]))
  load(fileNames[f])
  tab[f, 5:7] <- round(colMeans(rslt$gp.mat < 0.05), 3)
}
