...
# rm(list = ls())
# setwd("~/Project/QH0020/medtest/PhyloMed/Simulation/continuous/GETSIM/shared/")
# ########## input arguments
...
if(all(A==1, B==1)){
  tmp.js.asym = .calEFDRnDR(rslt.phylomed$PhyloMed.A$node.pval.js, causalNode, random.id-K)
  tmp.jsmix.asym = .calEFDRnDR(rslt.phylomed$PhyloMed.A$node.pval.jsmix, causalNode, random.id-K)
  tmp.js.perm = .calEFDRnDR(rslt.phylomed$PhyloMed.P$node.pval.js, causalNode, random.id-K)
  tmp.jsmix.perm = .calEFDRnDR(rslt.phylomed$PhyloMed.P$node.pval.jsmix, causalNode, random.id-K)
  tmp.sobel = .calEFDRnDR(rslt.phylomed$PhyloMed.A$node.pval.sobel, causalNode, random.id-K)
  
  tmp = rbind(tmp.js.asym, tmp.jsmix.asym, tmp.js.perm, tmp.jsmix.perm, tmp.sobel)
  efdr = tmp[,1] # empirical FDR
  dr = tmp[,2] # discover rate on the most common ancestor
  names(efdr) = names(dr) = c("js.asym", "jsmix.asym", "js.perm", "jsmix.perm", "sobel")
}else{
  efdr = dr = NULL
}
