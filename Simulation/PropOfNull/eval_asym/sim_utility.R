...
.pi0_JC.optimalRate <- function(z){
  ...
  for (j in 1:length(tt)) {
    t = tt[j]
    f = exp(t^2/2)
    epshat = mean(f*cos(t*z))
    epsest = c(epsest,epshat)
  }
  tmp = epsest[floor(length(tt)/2)]
  if(tmp > 1) tmp = 1
  return(tmp)
}
...
.nullEstimation_true <- function (pval.alpha, pval.beta, alpha1, alpha2, alpha00, alpha01, alpha10, alpha11) {
  # alpha00: a=0 and b=0
  # alpha01: a=0 and b!=0
  # alpha10: a!=0 and b=0
  ...
  rslt = list(alpha10 = alpha10, alpha01 = alpha01, alpha00 = alpha00, alpha11 = alpha11, 
              alpha1 = alpha1, alpha2 = alpha2, rawp = rawp.wNA)
  return(rslt)
}

.nullEstimation_prod <- function (pval.alpha, pval.beta, alpha1, alpha2, z1, z2, est.method) {
  # alpha00: a=0 and b=0
  # alpha01: a=0 and b!=0
  # alpha10: a!=0 and b=0
  #alpha1=alpha1.est; alpha2=alpha2.est; z1=z.stat.alpha; z2=z.stat.beta; rescale = TRUE;est.method = "JC.max"
  input.pvals = cbind(pval.alpha, pval.beta)
  input.z = cbind(z1, z2)
  idx.na = which(!complete.cases(input.pvals))
  input.pvals = input.pvals[complete.cases(input.pvals), ]
  ...
  if(est.method == "prod"){
    ...
    alphastar = min(alpha1 + alpha2 - alpha00, 1)
    alpha11 = max(1-alpha00-alpha01-alpha10, 0)
  }else if(est.method == "pmin"){
    alpha00 = .pi0_JC(z.max)
    alpha10 = max(alpha2-alpha00, 0)
    alpha01 = max(alpha1-alpha00, 0)
    alphastar = min(alpha00+alpha10+alpha01,1)
    alpha11 = max(1-alpha00-alpha01-alpha10, 0)
  }else if(est.method == "pmax"){
    w = .pi0_JC(z.min)
    alphastar = min(w, 1)
  ...
  rslt = list(alpha10 = alpha10, alpha01 = alpha01, alpha00 = alpha00, alpha11 = alpha11, alphastar = alphastar,
              alpha10.r = alpha10.r, alpha01.r = alpha01.r, alpha00.r = alpha00.r,
              alpha1 = alpha1, alpha2 = alpha2, rawp = rawp.wNA)
  return(rslt)
}

.nullEstimation_pmax_multistage <- function (pval.alpha, pval.beta, alpha1, alpha2, z1, z2) {
  #alpha1=alpha1.est.jc;alpha2=alpha2.est.jc;z1=z.stat.alpha;z2=z.stat.beta
  input.pvals = cbind(pval.alpha, pval.beta)
  input.z = cbind(z1, z2)
  idx.na = which(!complete.cases(input.pvals))
  input.pvals = input.pvals[complete.cases(input.pvals), ]
  ...
  if(length(which(rawp==0))>0)
    rawp[rawp == 0] = runif(length(which(rawp==0)), min = 0, max = 1e-7)
  ...
  rslt = list(alpha10 = alpha10, alpha01 = alpha01, alpha00 = alpha00, alpha11 = alpha11, alphastar = alphastar.new,
              alpha10.r = alpha10.r, alpha01.r = alpha01.r, alpha00.r = alpha00.r,
              alpha1 = alpha1, alpha2 = alpha2, rawp = rawp.wNA)
  return(rslt)
}

.nullEstimation_minus <- function (pval.alpha, pval.beta, alpha1, alpha2, est.method) {
  # alpha00: a=0 and b=0
  # alpha01: a=0 and b!=0
  # alpha10: a!=0 and b=0
  # alpha1 = alpha1.est.jc; alpha2 = alpha2.est.jc
  input.pvals = cbind(pval.alpha, pval.beta)
  idx.na = which(!complete.cases(input.pvals))
  input.pvals = input.pvals[complete.cases(input.pvals), ]
  
  # if(est.method == "hdmt"){
  #   alpha00 = min(HDMT_estNullProp(input.pvals)$alpha00, 1)
  # }else 
  if(est.method == "pmax"){
    w = min(AverageEstimate(pmax(input.pvals[,1], input.pvals[,2])), 1)
    alphastar = w
    alpha00 = min(alpha1+alpha2-w, 1)
    alpha10 = max(w-alpha1, 0)
    alpha01 = max(w-alpha2, 0)
    alpha11 = max(1-w, 0)
  }else if(est.method == "pmin"){
    alpha00 = min(AverageEstimate(pmin(input.pvals[,1], input.pvals[,2])), 1)
    alpha01 = max(alpha1 - alpha00, 0)
    alpha10 = max(alpha2 - alpha00, 0)
    alphastar = min(alpha00+alpha01+alpha10, 1)
    alpha11 = max(1-alphastar, 0)
  }else if(est.method == "revised"){
    alpha00 = min(AverageEstimate_mod(pmin(input.pvals[,1], input.pvals[,2])), 1)
    alpha10 = max(alpha2-alpha00, 0)
    alpha01 = max(alpha1-alpha00, 0)
    alphastar = min(alpha00+alpha01+alpha10, 1)
    alpha11 = max(1-alphastar, 0)
