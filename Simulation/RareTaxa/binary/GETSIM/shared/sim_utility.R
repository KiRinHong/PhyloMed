phylomed <- function(treatment, mediators, outcome, tree, covariates = NULL, interaction = FALSE,  
  ...
  #treatment=Trt;mediators=M;covariates=NULL;interaction=FALSE;fdr.alpha=0.05;n.perm=1e5;perm.prec=0.1;verbose=TRUE
  M = mediators
  ...
  Trt.ori = treatment
  outcome.ori = outcome # outcome.type = ifelse(length(unique(outcome.ori)) > 2, "continuous", "binary")
  ...
  rslt = list(PhyloMed.A = list(global.pval = globalp.asym),
              PhyloMed.P = list(global.pval = globalp.perm))
  return(rslt)
}
