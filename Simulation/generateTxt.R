rm(list = ls())
# regular 
tmp <- do.call(expand.grid, list(type=1:2, nsample=c(50,200), sim=1:2000, A=c(0,1), B=c(0,1)))
...
tmp <- do.call(expand.grid, list(type=1:2, nsample=c(50,200), A=c(0,1), B=c(0,1)))
tmp <- tmp[-which(tmp$type == 2 & tmp$A == 0 & tmp$B == 0),]
